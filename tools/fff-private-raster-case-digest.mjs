...
const ACCEPTED_SOURCE_COMMIT = "2e96bd380d47869024587eeb19b3f054064390af";
const ACCEPTED_SOURCE_TREE = "accfb3fc5474f4ecb2e39b5c4d69fd8de6a7e841";
const ACCEPTED_SOURCE_PACKAGE_FINGERPRINT = "0f701e7cfa106dee19cf6e378eec1082920cd7f119f37be0f09696ac8020fbf2";
const ACCEPTED_MP4_SHA256 = "0fb679b5d13d56b726a505d060bf9678daa49a1c138e10657954cd7053765df1";
const INTEGRATION_CANDIDATE_ID = "fff-case-digest-accepted-forward-integration-v1";
const INTEGRATION_TARGET_REF = "refs/heads/codex/fff-case-digest-accepted-forward-integration-v1";
const INTEGRATION_BASE_COMMIT = "701869936943f9babd7d4d5287008b646106188b";
const INTEGRATION_BASE_TREE = "85a1ca377d655151a348e3e657f3ddc1c3e1177c";
const HUMAN_OBSERVATION_JA = "事件ダイジェストとして理解できます。字幕改行にも問題ありません。次へ進めてよいです。";
const ACCEPTED_C2_FIXED_HASHES = {
  review_caption_csv_sha256: "447512097ba63c63685e2fd5e8549c714fd853de8978cfd93b04322ec35c6f7d",
  selected_shot_sequence_csv_sha256: "fa50635ec9dc17dfed233bac9c28197644be91e986033c309c8f3019066b7ec2",
  subtitle_layout_evidence_csv_sha256: "cc7d02ac77d92e95ed94caef70ffa6b1905fb23369b5eb57f260a42ddaf855eb",
  transition_boundary_map_csv_sha256: "e67312d91f7d8d1e9231acdd419263b05bbcb78044eadb0bd2fcd4568dede5d3"
};
...
function humanAcceptanceDecision() {
  return {
    ...
    source_binding: {
      human_review_context_commit: HUMAN_REVIEW_CONTEXT_COMMIT,
      accepted_source_ref: ACCEPTED_SOURCE_REF,
      accepted_source_commit: ACCEPTED_SOURCE_COMMIT,
      accepted_source_tree: ACCEPTED_SOURCE_TREE,
      accepted_source_package_fingerprint: ACCEPTED_SOURCE_PACKAGE_FINGERPRINT,
      accepted_mp4_sha256: ACCEPTED_MP4_SHA256
    },
    ...
    inheritance_proof: {
      ...
      review_caption_wording_and_timing: "exact_C2",
      selected_shot_order_and_timing: "exact_C2",
      subtitle_layout_evidence: "exact_C2",
      source_image_identities: "exact_C2",
      html_visible_markup_style_playback_captions_and_layout: "exact_C2_except_embedded_state_and_decision_metadata",
      mp4_bytes: "exact_C2",
      fixed_payload_sha256: ACCEPTED_C2_FIXED_HASHES
    }
  };
}
...
async function buildModel(sourceBundle, sharp) {
  ...
  return {
    ...
    human_acceptance: humanAcceptanceDecision(),
    ...
    boundaries: {
      ...
      human_comprehension_review_performed: true,
      human_comprehension_review: "accepted_scoped_case_digest",
      accepted_creative_content_changed: false,
...
function contractFailures(model) {
  ...
  require(model.human_acceptance.excluded_dimensions.includes("production subtitle selection")
    ...
    && model.human_acceptance.excluded_dimensions.includes("product or public release"), "human acceptance scope expanded");
  ...
  require(!model.boundaries.production_approved
    && !model.boundaries.production_subtitle_selected
    && !model.boundaries.selected_for_production
    && !model.boundaries.rights_cleared_claim
    && !model.boundaries.public_release
...
これは無音のprivate review用CASE_DIGESTです。人による理解確認はCASE_DIGEST comprehension、review-caption wording/readability、既存visual sequenceに限って成立しています。production subtitle selection、production acceptance、rights clearance、公開、音声、final canonは成立していません。
`;
}
...
\`${ARTIFACT_ID}\` は、Product Owner指定のCASE_DIGESTへ物語の提示形式を置き換えた、無音・private・default-offの180秒successor candidateです。
...
\`${HUMAN_VERDICT_ID}\` は「${HUMAN_OBSERVATION_JA}」を、CASE_DIGEST comprehension、review-caption wording/readability、変更のない既存visual sequenceに限って受け入れています。継承元は \`${ACCEPTED_SOURCE_COMMIT}\`、MP4 SHA256は \`${ACCEPTED_MP4_SHA256}\` です。accepted creative contentの変更は0です。
...
function renderQuarantineJson() {
  return {
    ...
    replacement_candidate_artifact_id: ARTIFACT_ID,
    replacement_status: "ACCEPTED_SCOPED_DEFAULT_OFF_SUCCESSOR_CANDIDATE",
    human_acceptance: humanAcceptanceDecision(),
    canonical_integration_boundary: {
      integration_candidate_id: INTEGRATION_CANDIDATE_ID,
      target_ref: INTEGRATION_TARGET_REF,
      parent_commit: INTEGRATION_BASE_COMMIT,
      accepted_source_commit: ACCEPTED_SOURCE_COMMIT,
...
async function captureBrowserEvidence(model) {
  ...
  try {
    const inspect = async (name, width, height, screenshotPath, seekTo) => {
      ...
      return {
        ...
        observed_screenshot_capture_performed: observedScreenshot.length > 0
      };
    };
  ...
  return {
    passed: consoleErrors.length === 0
      ...
      && results.desktop.observed_screenshot_capture_performed
      && results.narrow.observed_screenshot_capture_performed
...
- verdict: \`${HUMAN_VERDICT_ID}\`
- observation: 「${HUMAN_OBSERVATION_JA}」
...
async function validate(inputPath = RESULT_PATH) {
  ...
  require(valuesEqual(model.human_acceptance, humanDecision), "model human acceptance mismatch");
  require(valuesEqual(result.human_acceptance, humanDecision), "result human acceptance mismatch");
  require(valuesEqual(manifest.human_acceptance, humanDecision), "package manifest human acceptance mismatch");
  require(valuesEqual(root.private_raster_case_digest?.human_acceptance, humanDecision), "root human acceptance mismatch");
  ...
  require(liveMp4.sha256 === ACCEPTED_MP4_SHA256, "accepted C2 MP4 identity changed");
  ...
  const fixedPayloadChecks = await Promise.all([
    [REVIEW_CAPTIONS_PATH, ACCEPTED_C2_FIXED_HASHES.review_caption_csv_sha256],
    [SHOT_SEQUENCE_PATH, ACCEPTED_C2_FIXED_HASHES.selected_shot_sequence_csv_sha256],
    [SUBTITLE_EVIDENCE_PATH, ACCEPTED_C2_FIXED_HASHES.subtitle_layout_evidence_csv_sha256],
    [TRANSITION_MAP_PATH, ACCEPTED_C2_FIXED_HASHES.transition_boundary_map_csv_sha256]
  ].map(async ([filePath, expected]) => sha256(await readFile(filePath)) === expected));
  require(fixedPayloadChecks.every(Boolean), "accepted C2 fixed payload identity changed");
