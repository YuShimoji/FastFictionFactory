...
Fast Fiction Factory is a local-first fiction production workbench. The current review checkpoint is `fff-home-cockpit-metric-linking-001`, served through the static local Visual Review Hub at `public/review/index.html`. It makes `public/review/index.html?mode=brief` the no-query Home Cockpit entry route, keeps `public/review/index.html?mode=home` as a compatibility alias, links readiness meters to concrete next actions, and preserves the Review Home Map `fff-review-home-map-meters-001`, Draft-to-Video Planning Bridge `fff-draft-to-video-planning-bridge-001`, Review Brief / dark-mode UX checkpoint `fff-review-brief-dark-mode-ux-001`, One-story Draft Review Pack `fff-one-story-draft-review-pack-001`, Designer Candidate Dashboard `fff-designer-candidate-dashboard-001`, stabilization checkpoint `fff-draft-review-pack-stabilization-001`, and contradictory-claim guard `fff-contradictory-claim-guard-001`.

## Current Lane

Keep the MVP reviewable without production commitments. The current lane is Home Cockpit followed by the Draft-to-Video Planning Bridge for the selected 3-minute mystery-lore route: operator reading, workbench fallback, evidence audit shelves, locked production lanes, rights-risk posture, and production non-goals must remain distinct before any model/API extractor, provider/API setup, AI video generation, production render, public upload, rights-clearance claim, or final canon path exists.
...
Latest Home Cockpit Metric Linking refresh started from synced `master` on 2026-07-06 JST with `HEAD...origin/master` clean before edits. The pushed commit after this line is discoverable with `git log -1 --oneline --decorate` after pulling.
