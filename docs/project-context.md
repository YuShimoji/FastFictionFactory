...
Fast Fiction Factory is a local-first fiction production workbench. The current review checkpoint is `fff-review-home-map-meters-001`, served through the static local Visual Review Hub at `public/review/index.html`. It makes `public/review/index.html?mode=home` the no-query entry route, maps the folded shelves with status meters and open triggers, and preserves the Draft-to-Video Planning Bridge `fff-draft-to-video-planning-bridge-001`, Review Brief / dark-mode UX checkpoint `fff-review-brief-dark-mode-ux-001`, One-story Draft Review Pack `fff-one-story-draft-review-pack-001`, Designer Candidate Dashboard `fff-designer-candidate-dashboard-001`, stabilization checkpoint `fff-draft-review-pack-stabilization-001`, and contradictory-claim guard `fff-contradictory-claim-guard-001`.

## Current Lane

Keep the MVP reviewable without production commitments. The current lane is a Review Home Map followed by the Draft-to-Video Planning Bridge for the selected 3-minute mystery-lore route: source evidence, routing, contradictory claim holds, review-held defaults, artifact governance, local access paths, rights-risk posture, and production non-goals must remain distinct before any model/API extractor, provider/API setup, AI video generation, production render, public upload, rights-clearance claim, or final canon path exists.
...
- The active Fast Fiction Factory review surface is now
  `fff-review-home-map-meters-001`; it records Review Home as the no-query
  entry, seven shelf cards, seven meters, Primary / Story / Evidence navigation
  grouping, Bridge one-click access, Evidence Vault open triggers, and closed
  production gates while preserving `fff-draft-to-video-planning-bridge-001`,
...
- Review Home Map Meters readback passes for `fff-review-home-map-meters-001`: no-query access defaults to `home`, seven shelf cards and seven meters are visible, six meters are measured and one is a UX hypothesis, Bridge is linked from the first screen, Evidence Vault shelves show open triggers, Dark / Light / Auto theme controls remain present, and provider/API, AI video, render, upload, final canon, and rights-clearance boundaries remain closed.
...
Latest Review Home Map refresh started from synced `master` on 2026-07-06 JST with `HEAD...origin/master` clean before edits. The pushed commit after this line is discoverable with `git log -1 --oneline --decorate` after pulling.
