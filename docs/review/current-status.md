...
- This translation policy boundary slice started clean on `master` at `9f7012e Add translation provenance readback`; the contradictory-claim guard was already the tracked active surface, so the downstream gate, downstream scope-lock, provider-envelope readiness, provider authorization readiness, multilingual fixture coverage, translated audit, translation provenance readback, and very broad audit remain preserved auxiliary readbacks instead of active-status rewinds.
...
- Remaining fixture classes are translated memo text and very broad source-span fixture shape. Translated memo text has been audited and now has both source-span/claim provenance and a source-of-truth boundary; it is still not implemented as a fixture because the next fixture should be a separate narrow slice that preserves original spans and declares translation provenance. Very broad source-span shape has been audited but not implemented because current broad rows are already resolved and no concrete source-output gap requires another fixture.
...
| Translated memo fixture audit | Covered by `fff-translated-memo-fixture-audit-001`; existing multilingual coverage audited, translated fixture count 0, translated memo text remains the next policy-bound fixture gap | Use the translation policy boundary before adding a translated fixture |
...
Use review memory before asking for another review. The provider authorization boundary is documented, and the translation source-of-truth boundary now exists, so the next non-redundant move is either a translated memo fixture that preserves original spans and declares translation provenance, or real provider adapter implementation only after explicit authorization for provider choice, credentials, endpoint, transport behavior, external call permission, timeout, and retry policy. Do not add a broad source-span fixture until source output changes or broad shape is proven to be the coverage bottleneck. Do not start model/API behavior, database persistence, publishing, AI video generation, production sync, credentials, or final canon decisions unless explicitly requested.
