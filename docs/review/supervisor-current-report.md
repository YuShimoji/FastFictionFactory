...
Fast Fiction Factory は、ローカル優先の fiction production workbench として、原稿候補の抽出・人間レビュー・編集差分・制作設計・制作引き渡しを、source / truth / rights / canon の境界を保ちながら一作品分つなげた状態です。現在の active artifact は `fff-beat2-composition-board-001`、active lane は `COMPOSITION_BOARD` です。

直近の開発目的は、Beat 2「真鍮の蛾」`00:20–00:50` の三つの source shot を、雰囲気参照から実際のcrop / focal order / depth / placement判断へ一段具体化し、プロジェクト外の制作者が三つを別の画面として説明できるか判定することです。Visual Treatment Pilot、Storyboard Brief、Production Execution Pack、wording、timing、ID、truth、rights、selection、canon は変更していません。
...
ただし、構図が人へ伝わるという主張はまだ推測です。次の正規 gate は standalone Composition Board だけを使う H1 freeform composition-transfer review です。2/3以上のshotがpredecessorよりstageしやすくならない場合は `COMPOSITION_SPECIFICITY_BLOCKER` とし、AI image generationを開きません。production asset selection、rights clearance、full-story expansion、engine / voice、provider/API、credentials、external calls、art/audio/video generation、render、upload、publication、database persistence、production approval、final canon は引き続き closed です。

## ライブ同期と開発可能性

| 項目 | 2026-07-15 JST の確認結果 |
| --- | --- |
...
| Current authority | `docs/project-context.md` → `docs/review/current-status.md` → 本報告 → Composition Board review/result |
...
| Gate | Result |
| --- | --- |
| Root manifest read-only validation | Composition → Visual Treatment → Storyboard → Execution → Typography → Operator → Blueprint → Derivative → Revision → Handoff の順で実行 |
| Beat 2 Composition validator | `passed=true`、`failures=[]`、22/22 probes を完了条件とする |
| Tool syntax | pass。`fff-state.mjs`、Beat 2 generator、Storyboard generator |
| MkDocs | `uvx --with mkdocs-material mkdocs build --strict` pass。build output は一時ディレクトリへ隔離 |
| Launcher | `open_review.ps1 -Mode blueprint -PrintUri` pass |
| Patch hygiene | `git diff --check` pass |
| Read-only property | 上記 health check 後の tracked / untracked product change なし |
...
- Next move: 3 shot中2以上がpredecessorよりstageしやすいか判定し、pass、bounded revision notes、または`COMPOSITION_SPECIFICITY_BLOCKER`を記録。
...
- Next move: H1実施後、Boardをaccepted、bounded repair、またはblockedとして明示。
...
1. 直近を G0 Beat 2 H1 に限定し、Storyboard Brief 全体のH1を競合入口にしないこと。
2. H1 pass後、full 19-shot expansionへ直行せず、G2 second-beat falsificationを一回挟むか判断すること。推奨は挟む。
...
推奨は、まず standalone Beat 2 page だけで H1 を実施し、pass後に一つの対照的なbeatでpatternを反証テストする経路です。これにより、Beat 2固有の成功を19shot全体へ早まって一般化せず、source contractとrights/canon boundariesを保ったまま、offline assemblyへ最短距離で進めます。
...
H1 review contract は `docs/review/beat2-visual-treatment-pilot.md`、live H0 result は `artifacts/beat2-visual-treatment-pilot-result.json` を正本とします。通常のhealth checkでは `smoke-*` regenerationを使わず、root manifestのread-only validationを使います。
